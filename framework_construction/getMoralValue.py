...
def get_mfd(pair_path):
    ...
    # Create the label_map that maps index to corresponding dimension and value
    label_map = {
    ...
    for line in lines[12:]:
        # Split the line into parts, the last part is the index, the rest is the word/phrase
        parts = line.strip().rsplit(maxsplit=1)
    
        word = parts[0]  # This includes everything before the last space
        if len(word.split()) > 1:
            continue 
        if word not in cues:
            continue
        label_index = int(parts[1])  # The last part should be an integer
...
def build_graph(pair_path):
    ...
    #adjacency_matrix = torch.tensor(adjacency_matrix, dtype=torch.float32, device='cuda')  # vocab_size, vocab_size
    adjacency_matrix = tt.FloatTensor(adjacency_matrix) 
    initial_matrix = tt.FloatTensor(initial_matrix)  # vocab_size, 5 (for the 5 moral dimensions)
...
def normalize_trans_matrix(adjacency_matrix):
    ...
    #trans_matrix = torch.linalg.multi_dot([out_degree_matrix, adjacency_matrix, in_degree_matrix])
